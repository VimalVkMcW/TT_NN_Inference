...
def test_conv_dropout_layernorm_layers(layer_idx, squeezebert_model):
    ...
    new_state_dict = PCC.modify_state_dict_with_prefix(model, f'encoder.layers.{layer_idx}.output.')

    model = ConvDropoutLayerNorm(3072, 768, 4, 0)
    model_R = ConvDropoutLayerNorm_R(3072, 768, 4, 0)

    model.load_state_dict(new_state_dict)
    model_R.load_state_dict(new_state_dict)

    hidden_states = torch.rand(1, 3072, 8)
